...
public class GoogleUserRepository : IGoogleUserRepository
{
    ...
    public async Task UpdateUserTokensAsync(Guid userId, string newAccessToken, DateTimeOffset expiresAt, string newRefreshToken)
    {
        var user = await _context.GoogleUsers.FindAsync(userId);
        if (user == null)
        {
    ...
    public async Task SetLastDataUpdateDateTime(GoogleUser user)
    {
        user.LastDataUpdate = DateTime.UtcNow;
        await _context.SaveChangesAsync();
    }
}